            ...
            <ul class="navbar-nav ms-auto align-items-center" style="font-family: 'Poppins', sans-serif; font-size: 20px; font-weight: bold;">
                ...
                <li class="nav-item dropdown px-1">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarProfile" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <img src="{{ asset('img/title/consumer.png') }}" alt="Profile Image" class="rounded-circle" style="width: 30px; height: 30px; margin-right: 8px;">
                            @else
                            <img src="{{ auth()->guard('user')->user()->profile_pic ? asset(auth()->guard('user')->user()->profile_pic) : asset('img/title/consumer.png') }}" alt="Profile Image" class="rounded-circle" style="width: 30px; height: 30px; margin-right: 8px;">
                            @endif
                    </a>
                    ...
                    <ul class="dropdown-menu dropdown-menu-end border-0" aria-labelledby="navbarProfile">
                        ...
                        <li class="text-center my-2">
                            <img src="{{ auth()->guard('user')->user()->profile_pic ? asset(auth()->guard('user')->user()->profile_pic) : asset('img/title/consumer.png') }}"
                            alt="Profile Image"
                ...
                @else
                <li class="nav-item px-1">
                    <a class="nav-link" @if(request()->is('user/consumer/login')) active @endif href="{{ route('user.index') }} " data-page="login">LOGIN</a>
                </li>
                {{-- <li class="nav-item px-1">

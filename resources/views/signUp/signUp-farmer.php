                        ...
                        <table cellspacing='0' cellpadding='0' style='width: 400px; border-collapse: collapse; border: 1px solid #dddddd; border-radius: 8px; overflow: hidden;'>
                            <tr>
                                <td align='center' bgcolor='#ffffff' style='padding: 20px;'>
                                    ...
                                    <p style='font-size: 16px; font-family: Arial, sans-serif; color: #333333; margin-bottom: 30px;'>An E-Commerce Agri market for your business growth!</p>
                                    <p style='font-size: 16px; font-family: Arial, sans-serif; color: #333333; margin-bottom: 10px;'>Verification Code for Registration</p>
                                    <p style='font-size: 14px; font-family: Arial, sans-serif; color: #333333; margin-bottom: 20px;'>Check the OTP below fellow farmer</p>
...
<?php
 ob_start(); // Start output buffering
                ...
                $filter_name = htmlspecialchars($_POST['name']);
                ...
                $filter_pass = htmlspecialchars($_POST['pass']);
                $password = mysqli_real_escape_string($conn, $filter_pass );
                $pass=md5($password);
                $filter_cpass = htmlspecialchars($_POST['cpass']);
                ...
                $user_dir = '../uploads/' . $name;
                if (!file_exists($user_dir)) {
                    mkdir($user_dir, 0777, true);
                }

                // File paths
                $document_path = $user_dir . '/' . basename($document['name']);
                $card_path = $user_dir . '/' . basename($card['name']);


                // Move uploaded files to the uploads directory
                if (move_uploaded_file($document['tmp_name'], $document_path) && move_uploaded_file($card['tmp_name'], $card_path)) {
                    ...
                    if (mysqli_num_rows($select_users) > 0) {
                        ...
                        if ($pass != $cpass) {
                            ...
                            if ($insert_temp_verify) {
                                header('Location: verify_Fregister.php');
                                exit();
                            } else {
            ...
            if (isset($_GET['verified']) && $_GET['verified'] == 1) {
                if (isset($_SESSION['registration'])) {
                    ...
                    $card_path = $_SESSION['registration']['card'];
                    $document_path = $_SESSION['registration']['document'];
            
                    $insert_user = mysqli_query($conn, "INSERT INTO `user`(username, email, user_type, password,verify_status) VALUES('$name', '$email', '$type', '$pass',0)") or die('query failed');
                    if ($insert_user) {
                        // Get the last inserted id
                        $user_id = mysqli_insert_id($conn);

                        $select_farmer_details = mysqli_query($conn, "SELECT * FROM `farmer_details` WHERE user_id = '$user_id'") or die('query failed');

                        if (mysqli_num_rows($select_farmer_details) > 0) {                          
                        
                         //Add something here for error shit
                        } else {
                            $insert_farmer_details = mysqli_query($conn, "INSERT INTO `farmer_details` (user_id, phoneNum, identificationForm, identificationCard) VALUES ('$user_id', '$num', '$document_path', '$card_path')") or die('query failed');                                 
                            if ($insert_farmer_details) {
                                $_SESSION['message']="you have registered Sucessfully!";                          
                                header('Location: ../login/farmer-login.php');
                                exit();
                            }
                        }
                    }
...
<?php include '../navBar/navbar-farmer.php'?>
    ...
    <div class="container-fluid custom-font-content" style="padding: 20px;">
        ...
        <form id="signupForm" method="post" class="my-3" style="width: 100%;" autocomplete="off" enctype="multipart/form-data">
            <div class="row">
                ...
                <div class="col-lg-4 offset-lg-2 mb-4">
                    <div class="form-group my-3">
                        ...
                        <div class="input-group">
                            <input type="password" name="pass" class="form-control" id="password" placeholder="Enter password" minlength="8" required>
                    ...
                    <div class="form-group my-3">
                        ...
                        <div class="input-group">
                            <input type="password" name="cpass" class="form-control" id="password2" placeholder="Re-enter password" required>
                        </div>
            ...
            <div class="row">
                <div class="col-lg-4 offset-lg-1 mb-4">
                    <div class="container p-0">
                        ...
                        <input type="file" name="document" class="form-control-file" id="fileInput" name="fileInput" required>
                    </div>
                </div>
                <div class="col-lg-4 offset-lg-2 mb-4">
                    <div class="container p-0">
                        <label>Valid Identification(card):</label>
                        <input type="file" name="card" class="form-control-file" id="fileInput" name="fileInput" required>
                    </div>
                </div>
            </div>
            <!-- Submit Button -->
        ...
        <div class="row">
            <div class="col text-end mt-3 my-4">
                <a href="farmer-login.php">Create Account | Sign Up</a>
            </div>
        </div>
    </div>

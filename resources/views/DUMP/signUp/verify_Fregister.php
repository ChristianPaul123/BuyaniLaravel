<?php
//This is for the Farmer Registration
...
if (!isset($_SESSION['LOGIN'])) {
    ...
    header('Location: signUp-farmer.php');
    exit();
}
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (strtotime($otp_expiry) > time()) {
            if ($entered_otp == $stored_otp) {
                ...
                header('Location: signUp-farmer.php?verified=1');
                exit();
            } else {
                // OTP is incorrect
                $_SESSION['message']="OTP invalid Please try again!";
            }
        } else {
            // OTP is expired
